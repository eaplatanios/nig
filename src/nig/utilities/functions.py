...
class InheritDoc(object):
    """Docstring inheriting method descriptor. This class can also used as a
    decorator, although an alias called `inherit_doc` is provided for that.
    ...
    Bar.foo.__doc__ == Bar().foo.__doc__ == Foo.foo.__doc__ == "foo docstring"
    ```
    """
    def __init__(self, method):
        """Constructs a docstring inheriting method descriptor.

        Args:
            method (func): Method that inherits docstring.
        """
        self.method = method
    ...
    def use_parent_doc(self, func, source):
        if source is None:
            raise NameError('Cannot find "%s" in parents' % self.name)
