...
class Learner(with_metaclass(abc.ABCMeta, object)):
    def __init__(self, models, new_graph=False, session=None,
    ...
    def predict(self, data, pipelines=None, ckpt=None, working_dir=os.getcwd(),
        ...
        return self.session.run(
            fetches=self._postprocessed_output_ops, feed_dict=feed_dict)
    ...
    def predict_iterator(self, data, pipelines=None, yield_input_data=False,
        ...
        for data_batch in data:
            ...
            if not yield_input_data:
                ...
                yield data, self.session.run(
                    fetches=self._postprocessed_output_ops, feed_dict=feed_dict)


class SimpleLearner(Learner):
    """Used for training a single TensorFlow model."""
    def __init__(self, model, new_graph=False, session=None,
                 predict_postprocess=None, logging_level=0):
...
#                     if self.logging_level > 0:
#                         logger.info('Model %d finished training: Maximum '
#                                     'number of iterations reached.' % m)
#                     untrained_models.remove(m)
#                 prev_loss[m] = loss
